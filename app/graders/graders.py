...
def grade_task(state: dict) -> float:
    ...
    diagnosis_score = 1.0 if state.get("diagnosis") == task.diagnosis_target else 0.0
    validation_score = 1.0 if task.validation_target in state.get("validations_passed", []) else 0.0
    communication_score = 1.0 if task.communication_target in state.get("communications_sent", []) else 0.0
    penalties = min(
        0.25,
